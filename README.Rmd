...
The following example shows how Fuzzy DBScan works on the `multishapes` data set
...
implies that we need at least three points to detect a fuzzy core point.
...
Next, we train the DBScan based on data, ep and pts.
This is done by initialization of the `R6` object.
`FuzzyDBScan` contains a scatter plot method, where the clusters (colours) and
fuzzieness (transparency) are plotted for two features of choice.
...
Additionally, `FuzzyDBScan` is equipped with a prediction method.
This method freezes the algorithm such that new data points are not used for 
updating the cluster structure itself.
Each new data point is then assined a cluster and fuzziness individually by the 
